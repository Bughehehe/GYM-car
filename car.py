...
def run(episodes, is_training=True, render=False):
    ...
    # Divide position and velocity into segments || Infinite dx to small x
    
    pos_space = np.linspace(env.observation_space.low[0], env.observation_space.high[0], 25)    # Between -1.2 and 0.6
    vel_space = np.linspace(env.observation_space.low[1], env.observation_space.high[1], 25)    # Between -0.07 and 0.07

    if(is_training):
        q = np.zeros((len(pos_space), len(vel_space), env.action_space.n)) # init a 25x25x3 array
    else:
    ...
    plt.ylabel("Mean rewad [from 100 samples]")
    plt.xlabel("Episodes")
    plt.savefig(f'mountain_car.png')

if __name__ == '__main__':

    # run(10, is_training=True, render=False)
